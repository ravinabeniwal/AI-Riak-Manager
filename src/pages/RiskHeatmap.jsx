...
function RiskHeatmap() {
  ...
  const grid = LEVELS.slice().reverse().map(
    (p) => LEVELS.map((i) => ({
      probability: p,
      impact: i,
  ...
  return <div className="space-y-6">
        ...
        <div className="min-w-[560px]">
          <div className="flex">
            ...
            <div className="flex-1 grid grid-cols-4 gap-2 mb-2">
              {LEVELS.map((i) => <div key={i} className="text-center text-xs text-slate-500 font-mono uppercase">{i} Impact</div>)}
            </div>
          </div>
          {grid.map((row, rIdx) => <div key={rIdx} className="flex items-stretch gap-2 mb-2">
    ...
    >
      ...
      {selectedCell && <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-2xl p-6">
