...
function DashboardLayout({ children }) {
  ...
  return <div className="min-h-screen bg-[#060a16] noise-radial grid-bg flex">
      ...
      <aside
    className={`fixed md:sticky top-0 left-0 h-screen w-64 glass-strong border-r border-white/5 flex flex-col z-30 transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}
  >
    ...
    className={({ isActive }) => `flex items-center justify-between gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium transition-all ${isActive ? "bg-gradient-to-r from-teal-500/15 to-blue-500/10 text-teal-300 border border-teal-500/20" : "text-slate-400 hover:text-slate-200 hover:bg-white/5 border border-transparent"}`}
  >
              <span className="flex items-center gap-3">
                <Icon size={17} />
                {label}
              </span>
              {label === "Notifications" && unread > 0 && <span className="bg-red-500 text-white text-[10px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center px-1">
                  {unread}
                </span>}
            </NavLink>)}
        </nav>
