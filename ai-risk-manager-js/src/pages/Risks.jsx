...
function Risks() {
  ...
  const [form, setForm] = useState({ title: "", category: "", description: "", probability: "Medium", impact: "Medium", owner: "" });
  ...
  />
      ...
      <div className="glass rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.length === 0 && <tr><td colSpan={7} className="px-5 py-10 text-center text-slate-600">No risks match your filters.</td></tr>}
              {filtered.map((r) => <tr key={r.id} className="border-b border-white/5 last:border-0 hover:bg-white/[0.03] transition-colors">
      ...
      {showCreate && <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 px-4" onClick={() => setShowCreate(false)}>
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={form.probability} onChange={(e) => setForm((f) => ({ ...f, probability: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-slate-200">
                    {["Low", "Medium", "High", "Critical"].map((s) => <option key={s}>{s}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm text-slate-400 mb-1.5">Impact</label>
                  <select value={form.impact} onChange={(e) => setForm((f) => ({ ...f, impact: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-slate-200">
                    {["Low", "Medium", "High", "Critical"].map((s) => <option key={s}>{s}</option>)}
                  </select>
                </div>
              </div>
