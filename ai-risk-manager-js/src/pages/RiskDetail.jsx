...
function RiskDetail() {
  ...
  return <div className="space-y-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="relative">
              ...
              <div>
                ...
                <ul className="space-y-1.5">
                  {risk.aiInsight.keyIndicators.map((k, i) => <li key={i} className="text-sm text-slate-300 flex gap-2"><span className="text-teal-400 mt-0.5">•</span> {k}</li>)}
                </ul>
              </div>
            </div>
          </div>
          ...
          <div className="glass rounded-2xl p-6">
            ...
            <ul className="space-y-2">
              {risk.evidence.map((e, i) => <li key={i} className="text-sm text-slate-300 bg-white/[0.03] rounded-lg px-3 py-2">{e}</li>)}
            </ul>
          </div>
          ...
          <div className="glass rounded-2xl p-6">
            ...
            <ul className="space-y-1.5">
              {risk.contributingFactors.map((f, i) => <li key={i} className="text-sm text-slate-300 flex gap-2"><span className="text-amber-400 mt-0.5">•</span> {f}</li>)}
            </ul>
          </div>
          ...
          <div className="glass rounded-2xl p-6">
            ...
            <ul className="space-y-2">
              {risk.recommendations.map((rec, i) => <li key={i} className="text-sm text-slate-300 flex gap-2">
          ...
          <div className="glass rounded-2xl p-6">
            ...
            <div className="space-y-3">
              {risk.mitigationActions.map((m) => <div key={m.id} className="bg-white/[0.03] rounded-xl p-4">
    ...
    className={`w-full text-left px-3.5 py-2.5 rounded-xl text-sm border transition-colors ${s === risk.status ? "border-teal-500/30 bg-teal-500/10 text-teal-300" : "border-white/10 text-slate-400 hover:text-slate-200 hover:bg-white/5"} disabled:cursor-default`}
  >
                  {s}
                </button>)}
            </div>
          ...
          <div className="glass rounded-2xl p-6">
            ...
            {editingOwner ? <form onSubmit={handleAssign} className="flex gap-2">
                <input
    autoFocus
    value={ownerInput}
  ...
  />
              ...
              </form> : <button onClick={() => setEditingOwner(true)} className="text-xs text-teal-400 hover:text-teal-300">Reassign owner</button>}
          </div>

          <div className="glass rounded-2xl p-6">
            <h3 className="font-display font-semibold mb-4">Timeline</h3>
            <div className="space-y-3">
              {risk.timeline.slice().sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map((h) => <div key={h.id} className="flex gap-3 text-sm">
