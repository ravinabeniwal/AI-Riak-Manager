...
function Mitigation() {
  ...
  function cycleStatus(riskId, actionId, current) {
    const idx = STATUSES.indexOf(current);
    const next = STATUSES[(idx + 1) % STATUSES.length];
    updateMitigationStatus(riskId, actionId, next, next === "Completed" ? 100 : void 0);
  }
  ...
  >
            ...
            {showAdd === r.id && <form onSubmit={(e) => handleAdd(e, r.id)} className="grid sm:grid-cols-2 gap-3 mb-4 bg-white/[0.03] rounded-xl p-4">
            ...
            <div className="space-y-2">
              {r.mitigationActions.map((m) => <div key={m.id} className="bg-white/[0.03] rounded-xl p-4">
