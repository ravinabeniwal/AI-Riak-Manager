...
const SEVERITY_COLORS: Record<string, string> = { Critical: "#ef4444", High: "#f59e0b", Medium: "#3b82f6", Low: "#34d399" };
...
export default function Dashboard() {
  ...
  const overdue = risks.flatMap((r) => r.mitigationActions).filter((m) => m.status !== "Completed" && new Date(m.dueDate) < new Date()).length;
  ...
  const trend = risks
    ...
    .map((r, i) => ({ name: `#${i + 1}`, score: r.score }));

  const topRisks = [...risks].sort((a, b) => b.score - a.score).slice(0, 5);
  const recentActivity = risks
    .flatMap((r) => r.timeline.map((t) => ({ ...t, riskId: r.id, riskTitle: r.title })))
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          {user?.role === "admin"
            ? "Platform-wide activity, users, and risk detection statistics."
            : "Your organization's current risk posture, powered by AI."}
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="glass rounded-2xl p-6">
          ...
          <div className="space-y-3">
            {topRisks.map((r) => (
              <Link key={r.id} to={`/app/risks/${r.id}`} className="flex items-center justify-between gap-3 p-3 rounded-xl bg-white/[0.03] hover:bg-white/[0.06] transition-colors">
        ...
        <div className="glass rounded-2xl p-6">
          ...
          <div className="space-y-3">
            {recentActivity.map((a) => (
              <div key={a.id + a.riskId} className="flex gap-3 text-sm">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-2xl p-6">
        ...
        <p className="text-sm text-slate-400 leading-relaxed">
          {critical > 0 ? (
            ...
            "No critical risks are currently open. "
          )}
