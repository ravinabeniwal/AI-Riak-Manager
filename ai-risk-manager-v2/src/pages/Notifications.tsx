  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        {unread > 0 && (
          <button onClick={markAllNotificationsRead} className="text-sm px-4 py-2 rounded-xl glass text-teal-300 hover:bg-white/10 transition-colors">
            Mark all as read
          </button>
        )}
      </div>
      ...
      <div className="space-y-3">
        ...
        {sorted.map((n) => {
          ...
          const Icon = cfg.icon;
          return (
            <div key={n.id} className={`glass rounded-2xl p-5 flex items-start gap-4 transition-colors ${!n.isRead ? "border-teal-500/20 bg-teal-500/[0.03]" : ""}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-4 mt-3">
                  ...
                  {!n.isRead && (
                    <button onClick={() => markNotificationRead(n.id)} className="text-xs text-slate-500 hover:text-slate-300 flex items-center gap-1">
