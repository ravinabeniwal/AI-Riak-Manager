import type { Risk, MitigationAction, TimelineEvent } from "../types";

function score(probability: string, impact: string): number {
  const map: Record<string, number> = { Low: 1, Medium: 2, High: 3, Critical: 4 };
  const raw = (map[probability] * map[impact]) / 16;
  return Math.round(raw * 100);
}

function severityFromScore(s: number): "Low" | "Medium" | "High" | "Critical" {
...
function mkTimeline(events: Array<[string, string, string]>): TimelineEvent[] {
  return events.map(([date, event, actor], i) => ({ id: `t${i}`, date, event, actor }));
}

function mkActions(actions: Array<Partial<MitigationAction> & { id: string; action: string }>): MitigationAction[] {
...
const RAW: Omit<Risk, "score" | "severity">[] = [
  {
    id: "RSK-1001",
    title: "Third-Party Data Exposure",
    category: "Vendor & Third-Party",
    description:
      "A key analytics vendor's misconfigured storage bucket may have exposed customer usage logs, including partial account identifiers, for an estimated 11-day window.",
    probability: "High",
    impact: "Critical",
    confidence: 92,
    status: "Mitigating",
    ...
    aiInsight: {
      summary:
        "This risk was escalated after cross-referencing the vendor's disclosure with internal access logs, which independently confirmed unauthorized read access during the disclosed window.",
      whyItMatters:
        "Customer trust and regulatory notification obligations are directly at stake; several affected accounts fall under GDPR scope.",
      keyIndicators: [
    ...
    potentialImpact:
      "Regulatory fines, mandatory breach notifications in multiple jurisdictions, and reputational damage affecting customer retention.",
  ...
  },
  {
    ...
    description:
      "Several production storage buckets in the primary cloud account were found with overly permissive IAM policies, allowing broader read access than required by any current workload.",
    probability: "Medium",
    impact: "High",
    confidence: 87,
    status: "Under Review",
  ...
  },
  {
    ...
    category: "Compliance",
    description:
      "An internal audit identified that data retention practices in the EU region do not fully align with updated regulatory guidance issued this quarter.",
    probability: "Medium",
    impact: "High",
    confidence: 81,
    status: "New",
  ...
  },
  {
    ...
    description:
      "Access review detected several dormant privileged accounts that were never deprovisioned after role changes over the past two quarters.",
    probability: "High",
    impact: "Medium",
    confidence: 88,
    status: "Mitigating",
  ...
  },
  {
    ...
    category: "Vendor & Third-Party",
    description:
      "A critical payment processing vendor has no contractually guaranteed failover partner, creating a single point of failure for transaction processing.",
    probability: "Low",
    impact: "Critical",
    confidence: 74,
    status: "Monitoring",
  ...
  },
  {
    ...
    category: "Privacy",
    description:
      "A newly launched feature collects device location data with consent language that may not meet updated regional privacy disclosure requirements.",
    probability: "Medium",
    impact: "Medium",
    confidence: 79,
    status: "Under Review",
  ...
  },
  {
    ...
    category: "Operations",
    description:
      "The primary data center region lacks a tested failover runbook for a full regional outage scenario; the last full failover drill was over 18 months ago.",
    probability: "Low",
    impact: "High",
    confidence: 70,
    status: "New",
  ...
  },
  {
    ...
    description:
      "A dependency scan identified a widely-used open-source package in the build pipeline with a maintainer inactivity pattern consistent with abandonment risk.",
    probability: "Medium",
    impact: "Medium",
    confidence: 76,
    status: "Monitoring",
  ...
  },
  {
    ...
    description:
      "An internally deployed classification model has not undergone bias re-evaluation since a training data refresh three months ago.",
    probability: "Medium",
    impact: "Medium",
    confidence: 83,
    status: "Under Review",
  ...
  },
  {
    ...
    category: "Finance",
    description:
      "A segregation-of-duties gap was identified in the vendor payment approval workflow, allowing a single role to both create and approve vendor payments above the standard threshold.",
    probability: "Low",
    impact: "High",
    confidence: 85,
    status: "New",
...
export const RISKS: Risk[] = RAW.map((r) => {
